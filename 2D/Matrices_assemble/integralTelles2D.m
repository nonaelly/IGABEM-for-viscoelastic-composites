function G = integralTelles2D(elemCoords, gloBasisIdx, collocCoords, xiParam, ...
...
srcXi = convertToParentCoordSpace(xiParam, range);
xiStar = srcXi^2 - 1;
gamBar = nthroot(srcXi * xiStar + abs(xiStar), 3) + ...
    nthroot(srcXi * xiStar - abs(xiStar), 3) + srcXi;

% Gauss points in parameter space
gXi = gptU(:);
xi = ((gXi - gamBar).^3 + gamBar * (gamBar.^2 + 3)) ./ (1 + 3 * gamBar.^2);
xiParamSpace = convertToParamSpace(xi, range);
...
jacobTelles = reshape(jacobTelles', [1 1 size(gptU, 1)]);

jacob = jacobXi * jacobParam .* jacobTelles; % The final jacobian we use

% Compute the G matrix using ElastKernelTelles2D
G = ElastKernelTelles2D(r, dr, drdn, const1, const2, const3, const4, numGauss, ...
...
%% Sub functions.
% =========================================================================
% Calculate the weak singular matrix.
function G = ElastKernelTelles2D(r, dr, drdn, const1, const2, const3, const4, ...
