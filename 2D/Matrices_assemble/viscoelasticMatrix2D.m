function [HVis1, HVis2] = viscoelasticMatrix2D(nurbsStr, numR, numS, appliedPts, dA)
% Function: viscoelasticMatrix2D
% Description: Assembles the viscoelastic system matrices for a 2D problem.
%
% Inputs:
...
for i = 1:numApplied
    ...
    for e = 1:numElem
        ...
        xi = 0;
        if i <= numU
            [xi, flag] = JudgeSingular2D(nurbsStr, xiParam, range, ip, id, el);
        end

        [HVis1Sub1, HVis2Sub1, HVis1Sub2, HVis2Sub2] = integralViscoelastic2D(elemCoords, ...
            gloIdx, sourcePt, xi, range, p, knotVec, weights, const1, const2, const3, ...
            const4, ptsR, ptsS, weightR, weightS, appliedPts, dA, flag, D);
