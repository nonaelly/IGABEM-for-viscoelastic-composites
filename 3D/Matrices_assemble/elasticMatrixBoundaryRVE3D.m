function [H, G, HO, GO] = elasticMatrixBoundaryRVE3D(nurbsStr, collocNew, ...
...
%
% Outputs:
%   H: Elastic matrix for U for the RVE's matrix (strong singular).
%   G: Elastic matrix for T for the RVE's matrix (weakly singular).
%   HO: Elastic matrix for U.
%   GO: Elastic matrix for T.
...
for i = 1:numU
    ...
    for e = 1:numElem
        ...
        % Accumulate Matrices
        HO(row, colU) = HO(row, colU) + HSubO;
        GO(row, colT) = GO(row, colT) + GSubO;
        G(row, colT) = G(row, colT) + GSubO * (id < 7);
    end
...
for i = 1:numU
    HO((i * 3 - 2):(i * 3), (i * 3 - 2):(i * 3)) = 0; % Set the diagonal (3x3) to 0
end
for i = 1:numU
    rowI = i * 3 - 2:i * 3;
    Hbii = -[sum(HO(rowI, 1:3:end), 2), sum(HO(rowI, 2:3:end), 2), ...
        sum(HO(rowI, 3:3:end), 2)];
    HO(rowI, rowI) = Hbii;
end
JumpMatrix = HO - H1;
HO = HO * (tranU);

for i = 2:max([nurbsStr.bouInd])
    col = 3 * indNumT(i) + 1:3 * indNumT(i + 1);
    Str = nurbsStr([nurbsStr.bouInd] == i);
    H1(:, col) = H1(:, col) * (1 - Str(1).E / E);
    JumpMatrix(:, col) = JumpMatrix(:, col) * (1 + Str(1).E / E);
end

H = JumpMatrix + H1; % Add the jump matrix and integral matrix together

H = H * (tranU); % The H matrix for control points
