function [HVis1_1, HVis2_1, HVis1_2, HVis2_2] = integralViscoelastic3D(elemCoords, ...
    gloBasisIdx, sourcePt, xi, eta, range, p, q, knotU, knotV, weights, const1, const2, const3, ...
    const4, gptU_r, gptV_r, gptU_s, gptV_s, gwtU_r, gwtV_r, gwtU_s, gwtV_s, appliedPts, dA, flag, D)
% integralViscoelastic3D: Computes the viscoelastic matrix H for both
...
if flag
    srcXi = convertToParentCoordSpace(xi, range(1:2));
    srcEta = convertToParentCoordSpace(eta, range(3:4));
    xiStar = srcXi^2 - 1;
    etaStar=srcEta^2 - 1;
    gamBarX = nthroot(srcXi * xiStar + abs(xiStar), 3) + ...
        nthroot(srcXi * xiStar - abs(xiStar) + srcXi, 3);
    gamBarE=nthroot( srcEta*etaStar + abs(etaStar) ,3) + ...
        nthroot(srcEta*etaStar - abs(etaStar)+ srcEta,3);
    xi = ((gptU_s(:) - gamBarX).^3 + gamBarX * (gamBarX.^2 + 3)) / (1 + 3 * gamBarX.^2);
    eta = ((gptV_s(:) - gamBarE).^3 + gamBarE * (gamBarE.^2 + 3)) / (1 + 3 * gamBarE.^2);
    xiParam = convertToParamSpace(xi, range(1:2));
    etaParam = convertToParamSpace(eta, range(3:4));
    gwtU = gwtU_s;
    gwtV = gwtV_s;
    jacobTelles = 9 * ((gptU_s(:) - gamBarX).^2) .* ((gptV_s(:) - gamBarE).^2) / ((1 + 3 * gamBarX.^2) * (1 + 3 * gamBarE.^2));
    jacobTelles = reshape(jacobTelles', [1, 1, size(gptU_s(:), 1)]);
else
    xiParam = convertToParamSpace(gptU_r(:), range(1:2));
    etaParam = convertToParamSpace(gptV_r(:), range(3:4));
    gwtU = gwtU_r;
    gwtV = gwtV_r;
end
...
N = zeros(1, (p+1) * (q+1), numGauss); % 'N_Gauss' pages. For each page, 1*(p+1)
dNu = N;
dNv = N;

for i = 1:numGauss
    [N(:,:,i), dNu(:,:,i), dNv(:,:,i)] = NURBS2DBasisDers([xiParam(i,:), etaParam(i,:)], p, q,...
        knotU, knotV, weights');
end

[jacobXi, normals, r, dr, drdn] = getKernelParameters3D(elemCoords, sourcePt, N, dNu, dNv);
...
[HVis1_1, HVis2_1] = ViscoKernelA3D(r, dr, drdn, const1, const4, appliedPts, ...
    sourcePt, numApplied, dA, numGauss, jacob, gwtU, gwtV, D);

%-----------------------------------------------------------
% The term without singularity.
gwtBU = gwtU_r;
gwtBV = gwtV_r;
xiParamB = convertToParamSpace(gptU_r(:), range(1:2));
etaParamB = convertToParamSpace(gptV_r(:), range(3:4));

numGaussB = length(xiParamB); % Number of Gauss points

NB = zeros(1, (p+1) * (q+1), numGaussB); % 'N_Gauss' pages. For each page, 1*(p+1)*(q+1)
dNBu = NB;
dNBv = NB;

for i = 1:numGaussB
    [NB(:,:,i), dNBu(:,:,i), dNBv(:,:,i)] = NURBS2DBasisDers([xiParamB(i,:), etaParamB(i,:)], p, q,...
        knotU, knotV, weights');
end

[jacobXiB, ~, rB, drB, drdnB] = getKernelParameters3D(elemCoords, sourcePt, NB, dNBu, dNBv);

jacobB = jacobXiB * jacobParam; % The final jacobian we use

[HVis1_2, HVis2_2] = ViscoKernelB3D(rB, drB, drdnB, const1, const4, numGaussB, jacobB, gwtBU, gwtBV, D);
...
% =========================================================================
% HVis1_1, HVis2_1.
function [HVis1, HVis2] = ViscoKernelA3D(r, dr, drdn, const1, const4, appliedPts, ...
    sourcePt, numApplied, dA, numGauss, jacob, gwtU, gwtV, D)
% ViscoKernelA3D: Calculates the viscoelastic kernel matrices HVis1 and HVis2
...
gwtU = reshape(gwtU(:), [1, 1, 1, numGauss]);
gwtUExp = repmat(gwtU, [1, 1, numApplied, 1]);
gwtV = reshape(gwtV(:), [1, 1, 1, numGauss]);
gwtVExp = repmat(gwtV, [1, 1, numApplied, 1]);

H1 = jacobExp.*Ttemp1.*gwtUExp.*gwtVExp;
H2 = jacobExp.*Ttemp2.*gwtUExp.*gwtVExp;
...
function [HVis1, HVis2] = ViscoKernelB3D(r, dr, drdn, const1, const4, numGauss, jacob, gwtU, gwtV, D)
...
gwtUExp = repmat(gwtUExp, [1, 1, D, 1]);
gwtVExp = reshape(gwtV(:), [1, 1, 1, numGauss]);
gwtVExp = repmat(gwtVExp, [1, 1, D, 1]);
...
HVis1 = sum(jacobExp .* Ttemp1 .* gwtUExp .* gwtVExp, 4);
HVis2 = sum(jacobExp .* Ttemp2 .* gwtUExp .* gwtVExp, 4);
